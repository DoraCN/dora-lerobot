...
from lerobot.teleoperators.keyboard.teleop_keyboard import KeyboardEndEffectorTeleop, PYNPUT_AVAILABLE
...
def main(cfg: GymManipulatorConfig) -> None:
    ...
    try:
        task_name = getattr(cfg.env, "task", "")
    except Exception:
        task_name = ""

    if teleop_device is None and isinstance(cfg.env, HILSerlRobotEnvConfig) and "Keyboard" in task_name:
        print(f"[hotfix] teleop_device is None; task={task_name}. PYNPUT_AVAILABLE={PYNPUT_AVAILABLE}")
        try:
            kcfg = KeyboardEndEffectorTeleopConfig(use_gripper=True)
            teleop_device = KeyboardEndEffectorTeleop(kcfg)
            teleop_device.connect()
            print("[hotfix] KeyboardEndEffectorTeleop instantiated and connected.")
        except Exception as e:
            print("[hotfix] Failed to init/connect KeyboardEndEffectorTeleop:", e)
